...
index_folder_path = f"VectorDatabase/{datasets_name}/ShadowModel"  
...
path_members = f"SamplesDatabase/{datasets_name}/ShadowModel/samples_member.json"
path_nonmembers = f"SamplesDatabase/{datasets_name}/ShadowModel/samples_nonmember.json"
...
for max_tokens in max_tokens_list:
    ...
    none_mem_mqa_results, llm_results_nonmember = mqa_attack(samples_nonmember[:1000], False)
    

    os.makedirs(f'{path_attack_result}/PredictValues_Seq_Shadow/', exist_ok=True)  


    
    predict_values = mem_mqa_results + none_mem_mqa_results  
    with open(f'{path_attack_result}/PredictValues_Seq_Shadow/predict_values_{max_tokens}.json', 'w') as f:
        json.dump(predict_values, f)
    
    label_values = [1] * 1000 + [0] * 1000
    with open(f'{path_attack_result}/PredictValues_Seq_Shadow/label_values_{max_tokens}.json', 'w') as f:
        json.dump(label_values, f)



    
    with open(f'{path_attack_result}/PredictValues_Seq_Shadow/LLMMemberOutputs_{max_tokens}.json', 'w') as f:   
        json.dump(llm_results_member, f)
    with open(f'{path_attack_result}/PredictValues_Seq_Shadow/LLMNonMemberOutputs_{max_tokens}.json', 'w') as f:   
        json.dump(llm_results_nonmember, f)
    


    os.makedirs(f'{path_attack_result}/AUC_ROC_Seq_Shadow/roc', exist_ok=True)  

    roc_save_path = f'{path_attack_result}/AUC_ROC_Seq_Shadow/roc'
